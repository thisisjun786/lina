...
export async function compareBatch(
	...
	hosts: HostEvidence[] = [],
	registryPath = join(dirname(resolve(output)), "run-registry.sqlite"),
	...
	const registry = new RunRegistry(resolve(registryPath));
	...
	try {
				...
				if (scoreExitCode !== 0) {
					...
					report.runs.push({
						...
						promptTokens: trace.requests.reduce(
							(sum, r) =>
								sum +
								(r.transport.kind === "ok" ? r.transport.usage.prompt : 0),
							0,
						),
		...
		terminal = report.runs.every(
			(run) => run.exitCode === 0 && run.scoreExitCode === 0,
		)
			? "completed"
			: "failed";
		return report;
		...
		try {
			if (registered) registry.finish(root, terminal);
		} finally {
			registry.close();
		}
	}
