...
| LIFE image/avatar integration | Frozen permitted sources and visual grants, original provider UUID, separate artifact/post/avatar receipts, legacy owner migration, quota/pin/history/restore and no-POST pause recovery. [070](plans/life/070_images_and_avatars.md) passed 3,357 source tests, 27 actual curl cases and independent reviews. Synthetic bytes do not qualify real likeness or a renderer. |
| Integrated LIFE and failure isolation | [080](plans/life/080_surfaces_and_acceptance.md) and [consumer guide](LIFE_ENGINE.md): one-installation task→event→directed relations→pictured post→viewer feedback→later scoped experience, same-ID replay, strict rejected-store isolation for bound chat/tasks and actual offline checkpoint/process recovery. The final 080 local gate passed 3,363 tests, 17,847 assertions, source checks, 11 HTTP cases and independent review; hosted CI remains a separate PR result. |
...
Automatic semantic history, selective live restore, external-service export, the Electron redesign, multi-message delivery, clipboard additions, desktop provisioning and the LIFE renderer require their own implementation evidence. Engine/image integration evidence is recorded in 070/080; it does not qualify these external surfaces. The source-aware memory tests use isolated data; they do not qualify existing installed memory data, real narrative quality, publication or image delivery. Planning documents are acceptance requirements, not evidence that these features ran.
