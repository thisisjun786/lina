...
LINA는 에이전트마다 하나의 장기 대화를 유지한다. 업무에는 개발·검색·리서치·글쓰기·자료 정리가 포함되며, 필요할 때 별도 Codex 작업에 맡긴다. 짧은 검색이나 자료 공유에 작업 생성을 요구하지 않는다. 프로바이더 계정과 모델 연결은 OpenCodex가 관리한다.
...
flowchart LR
  ...
  Assistant --> LCM[LINA 원문 저널 · 세션 컨텍스트]
  Assistant --> Persona[관계 · 페르소나 기억]
  Assistant --> Work[LINA 공유 자료 · 업무 기억]
  Tasks --> Work
```
...
Codex 자체 압축과 LINA의 세션 컨텍스트 엔진은 별개다. LCM에서 참고한 원문·계층 요약·복원 원리를 LINA 저장소와 정책으로 구현했다.

- Codex는 모델에 넣을 실행 컨텍스트를 압축한다.
- LINA는 대화 원문을 보존하고, 원문 ID에 연결된 요약 트리를 만든다. 이전 요약과 새 원문을 함께 요약하므로 과거 결정을 따라갈 수 있다.
- 새 턴에는 활성 요약을 참고 자료로 넣는다. `lina_history_search`와 `lina_context_expand`는 원문과 요약의 근거를 찾아 펼친다.
- LINA 요약 체크포인트를 Codex의 네이티브 압축 영수증으로 위장하지 않는다. 요약 실패 시 직전 체크포인트와 원문을 유지하고 복구 필요 상태를 표시하면서 대화는 계속한다.
...
`startPersistentApp`은 신뢰된 세계 저장소를 돌려주는 `world` 함수를 받는다. Fleet는 저장된 에이전트 연결을 매번 읽어 이 함수를 제공한다. 예전의 인라인 세계 설정 객체는 초기화 전에 거부한다. 관리 API `GET/PUT /api/life/agents/:agentId/binding`에서 세계와 공개 수신자를 명시하며, 수신자가 없으면 성장만 공유한다. `lina_world_read`는 현재 허용된 사건만 명시적으로 읽고, 확인 모드에서도 읽기 승인을 기다리지 않는다.

저널은 요청·메시지·네이티브 세대와 실제 노출 기록을 연결한다. LIFE가 섞인 요청과 출처를 확인할 수 없는 과거 기록은 일반 사용자 사실을 학습하는 근거로 쓰지 않는다. 기억·선호·성찰·요약·작업 메모는 원래 출처 증거를 보존하고, 실제 전송과 적용 직전에 다시 검사한다. 과거 데이터는 보존하되 출처가 없는 값을 정상 기억으로 추정해서 복구하지 않는다. 상세 계약과 검증 증거는 [050](plans/life/050_work_and_persona.md)에 있다. 합성 응답 서버에서 확인한 전달·저장·재시작 결과는 실제 모델의 표현 품질이나 운영 설치본의 활성화 증거와 구분한다.
...
기억·페르소나·공유 자료는 LINA가 소유한다. 신규 설정의 기억 기본값은 `native`다. `disabled`는 자동 학습을 중지하고 저장된 로컬 기억을 보존한다. 과거의 `LINA_MEMORY_BACKEND=honcho` 설정은 이전 필요 상태를 반환하며 외부 서버에 접속하지 않는다. 외부 데이터가 로컬 기억으로 자동 이전됐다고 해석하지 않는다.
...
내부 기억·요약·자료 처리에는 공통 quick/standard/deep/intensive 라우트를 사용한다. 모델·추론 수준·출력 한도의 실제 적용값을 검증하며 일반 대화의 지정 모델은 유지한다. LIFE step과 새 게시 시도는 해석된 모델 선택을 저장한다. 기존 게시 기록은 당시 요청 형식을 유지하고, 현재 선택과 달라진 미전송 요청은 실행하지 않는다. 전체 인수와 실제 모델 품질 검증은 아직 완료하지 않았다.
...
LIFE 게시물과 프로필 사진은 기존 이미지 작업 소유자에 연결한다. 허용된
장면·외형을 요청 전에 고정하며, 생성·파일 저장·게시물 첨부·아바타 적용은
각각 기록한다. 같은 요청의 결과가 불명확하면 새 요청을 만들지 않는다.
일시정지는 새 생성과 자동 적용을 막고, 전송되지 않았음이 확인된 예약만
안전하게 해제한다. 고정·복원·수동 적용은 작성한 페르소나를 바꾸지 않는다.

`GET /api/life/health`는 설치 소유권이 있는 로컬 관리 요청에만 저장소의
`unopened`, `open`, `rejected` 상태를 반환한다. 이 조회는 DB나 모델을 열지
않는다. LIFE DB 열기가 거부되면 `LIFE_STORAGE_UNAVAILABLE` 상태를 유지하며
자동으로 다시 열거나 수정하지 않는다. 해당 세계에 연결된 에이전트도 세계
컨텍스트 없이 일반 대화와 업무를 계속할 수 있다. 모든 세계가 같은 LIFE
DB를 쓰므로 손상된 DB 내부의 다른 세계만 별도로 복구했다고 주장하지 않는다.

API·공개 대상·이미지 상태와 오프라인 체크포인트 복원 계약은
[LIFE 엔진 가이드](LIFE_ENGINE.md), 통합 검증은 [080](plans/life/080_surfaces_and_acceptance.md)에 있다.
LIFE 화면과 실제 모델·이미지 품질은 각각의 담당 영역에서 검증한다.
