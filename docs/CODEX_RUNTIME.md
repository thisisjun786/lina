...
LIFE 상태는 세계 DB에 저장한다. 일반 대화는 선택한 세계의 허용된 성격·습관·태도만 읽고, 사건 원문과 비밀은 자동으로 받지 않는다. LIFE 세션은 해당 에이전트의 경험과 믿음을 읽는다. 작성한 정체성, 수동 성장 설정과 잠긴 항목이 성장 투영보다 우선한다.

명시적 컨텍스트 정책을 사용하는 세션은 목적, 에이전트·세계 연결과 공개 정책 버전을 v2 헤더에 저장한다. 연결 변경·해제나 공개 권한 변경으로 기존 입력을 재사용할 수 없으면 새 네이티브 컨텍스트를 만든다. Lina 세션 ID와 이전 저널은 보존하며, 이전 저널을 새 모델 입력으로 복사하지 않는다. 전송 전 노출 기록과 전송 결과, 네이티브 세대별 메시지 ID도 저장한다. 응답 확인을 잃어 실행 여부가 불명확하면 중복 생성을 멈춘다.
