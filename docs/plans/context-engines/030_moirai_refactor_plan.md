...
LINA는 이 엔진을 사용하는 제품이다. 전체 엔진은 모이라이 시스템, 세 판단을 종합하는 주체는 모이라이(Moirai)다. 클로토·라케시스·아트로포스는 전망·계획, 경험·가치, 의도·연속성을 각각 판단한다. 구체적인 외부 창작 설정의 서사나 출처 설명은 저장소에 도입하지 않는다.
...
| 주체 | 판단 기능 | 계산과 검증할 결과 |
| --- | --- | --- |
| 클로토 / Clotho | 전망·계획 | LLM 계획·예측과 도구/코드의 조건 검사; 실제 결과와 예측 오차 비교 |
| 라케시스 / Lachesis | 경험·가치 | 출처 있는 경험 조회와 개인별 MaleCNS 반응; 단서별 선호 학습·재독출 |
| 아트로포스 / Atropos | 의도·연속성 | 목표·약속 상태 전이와 충돌 검사; 수락된 계획의 지속·완료·변경 |
| 모이라이 / Moirai | 종합 | 근거 종류를 구분한 후보·해석·계획 제안; Host가 정책·현재성·실행을 확정 |

세 모듈은 한 개인의 정체성·기억·정서·목표를 읽으며 별도 인격이나 세 개의 저장 원본을 만들지 않는다. 모두 전체 행동 후보를 제안하고 필수 현재 입력은 같게 받는다. 첫 판단은 서로의 출력을 보지 않고 병렬 생성한다. 이후 공통 후보를 같은 snapshot에서 평가하고 모이라이가 해석·계획을 종합한다. Host가 기준 정책·선호 반영·확정·실행을 소유한다. [016의 판단 계약](../platform/016_neural_preference_contract.md#세-판단-모듈의-계약)을 따른다.

합의는 증거를 대신하지 않는다. 미해결 충돌은 추가 확인·허용된 검증·질문·보류로 처리한다. 일반 대화의 기본 LLM 수명은 3+1, 명시적 행동은 3+2에 공통 후보 평가가 붙는다. 필요한 추가 LLM 호출·도구·신경 계산 비용을 모두 기록한다. 판단 모듈은 허용된 읽기/계산 포트를 사용하고 저장·외부 효과·사용자 발송은 Host와 기존 owner가 수행한다.
...
| Assessment | snapshotId·mechanismRevision·완결된 의견·근거·후보와 모듈별 계산 결과; 세부 타입은 016 소유 |
