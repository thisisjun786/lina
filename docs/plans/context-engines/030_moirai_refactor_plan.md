...
LINA는 이 엔진을 사용하는 제품이다. 전체 엔진은 모이라이 시스템, 판단을 종합하는 기능은 모이라이(Moirai)다. 클로토·라케시스·아트로포스의 이름은 코드와 실행 기록의 식별자로 유지한다. 세 기능은 별개 인격을 연기하는 에이전트가 아니라 하나의 인지 과정에 조언을 제공하는 의식 모듈이다. 구분의 기준은 이름·성격·말투가 아니라 목적 설계, 근거 평가, 의도와 약속에 따른 선택이라는 판단 책임이다.
...
| 주체 | 우선 관점 | 기술 역할 |
| --- | --- | --- |
| 클로토 / Clotho — 설계자 | 목적 달성·가능성·대안 | 목적과 가능한 경로를 구성하는 인지 모듈 |
| 라케시스 / Lachesis — 분석가 | 근거·실제 결과·불확실성 | 믿을 수 있는 결론과 갱신 조건을 평가하는 인지 모듈 |
| 아트로포스 / Atropos — 결정자 | 현재 의도·약속·제약·실행 가능성 | 현재 선택과 보류 조건을 판단하는 인지 모듈 |
...
세 모듈은 전체 상황에 대한 판단·추천 행동·근거·중요한 불확실성이나 판단 변경 조건을 영어 내부 조언으로 전달하며, 완성된 사용자 답변 세 개를 작성하지 않는다. 원문 인용·식별자·값은 정확성이 필요한 경우 원형을 보존한다. 필요한 깊이는 판단할 문제에 맞추고 공통 문장 수 제한이나 형식적인 반론을 강제하지 않는다. 시스템 프롬프트도 영어로 작성하고 이름을 연기하는 지시를 넣지 않는다. 모이라이의 최종 사용자 응답만 사용자의 현재 발화와 명시적 언어 요청을 따른다.

모이라이는 기여자의 이름이나 인격을 알 필요 없이 의견의 내용과 원래 입력을 대조해 판단한다. 모듈 식별자와 세션 연결은 호스트의 진단 기록에 남기되 모델의 시스템 텍스트나 종합 입력의 역할 설명으로 전달하지 않는다. 의견에 인용된 원래 근거의 식별자는 보존한다. 사용자용 말투·길이·형식 조건은 최종 응답에 적용하며 내부 의견을 그대로 중계하지 않는다. [영문 프롬프트 정본과 초기 기획 대응](../../../scripts/qa/senpi-sdk/prompts/README.md)은 실제 QA 실행기가 직접 가져오는 원문을 가리킨다.

합의는 증거를 대신하지 않는다. 미해결 충돌은 추가 확인·작은 검증 행동·질문·보류로 처리한다. 기본 실행은 독립 판단 3회와 종합 1회이며 반박 라운드는 추가 호출이다. 조언 모듈이 직접 저장·도구 실행·사용자 발송을 하지 않도록 능력을 제한한다. 토큰·시간은 관측값이며 메시지 품질을 희생해 축소할 기준으로 삼지 않는다.

기존 Codex 기반 비교 실험은 [연구 근거](032_moirai_prompt_research.md), [지침 후보](033_moirai_prompt_candidates.md), [QA C/E 비교 경로](040_moirai_prompt_comparison.md)에 보존한다. 해당 경로의 로컬 합성 provider 검증과 현재 [Senpi SDK 실험](../../../scripts/qa/senpi-sdk/README.md)은 별도 증거다. 현재 프롬프트 정본은 위의 영어 인지 모듈 파일이며, 이전 후보를 현재 실행 지침으로 혼동하지 않는다. 두 경로 모두 제품 통합이나 인지 효용 qualification 완료를 뜻하지 않는다.
...
- **G5 자원 관측:** 전체 실험 예산이나 인지 응답의 출력 토큰 상한을 임의로 두지 않는다. 인지 요청은 `max_output_tokens`를 지정하지 않으며 제공자 고유 제한에 걸리면 실제 종료 사유를 기록한다. 기존 episode 6회·요청 120초의 실행 안전장치와 출력 길이는 구분한다. 내부 전문 호출·재시도·불완전 응답의 사용량도 합산한다. 자원 증가는 메시지 품질을 희생하거나 접근을 기각할 근거로 삼지 않는다. 병렬 실행은 적응형으로 조정하고 실패를 버리지 않는다.
