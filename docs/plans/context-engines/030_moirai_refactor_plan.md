...
**모이라이는 경험으로 자기 관점을 형성하고, 그 관점에 따라 행동하는 AI 개인을 유지하는 에이전트 코어다.** 상황을 이해하는 인지, 경험에 따른 선호, 목표와 약속의 지속을 함께 지원한다. 저장량·자연스러운 답변·여러 모델의 합의만으로 이 기능이나 추가 효용을 입증했다고 보지 않는다.

LINA는 이 엔진을 사용하는 제품이다. 전체 엔진은 모이라이 시스템, 세 판단을 종합하는 주체는 모이라이(Moirai)다. 클로토는 미래 성과·성장 가능성, 라케시스는 자신의 욕구·선호 충족, 아트로포스는 채택한 목표·약속·정체성의 연속성을 각각 추구한다. 세 모듈의 분리 이유는 고유 목표에 따라 다른 답을 낼 수 있게 하는 것이다. 구체적인 외부 창작 설정의 서사나 출처 설명은 저장소에 도입하지 않는다.
...
| 주체 | 고유 목표·책임 | 계산과 검증할 결과 |
| --- | --- | --- |
| 클로토 / Clotho | 미래 성과·성장 가능성 확보 | LLM 계획·예측과 도구/코드의 조건 검사; 실제 결과와 예측 오차 비교 |
| 라케시스 / Lachesis | 자신의 욕구·선호 충족 | 출처 있는 경험 조회와 개인별 MaleCNS 반응; 단서별 선호 학습·재독출 |
| 아트로포스 / Atropos | 채택한 목표·약속·정체성의 연속성 유지 | 목표·약속 상태 전이와 충돌 검사; 수락된 계획의 지속·완료·변경 |
| 모이라이 / Moirai | 서로 다른 목표의 판단 종합·선택 | LLM 해석 + 버전 있는 ArbitrationPolicy. 목표별 추천·우선/양보 이유·선택 규칙을 남기고 Host가 검증·원자 기록·실행 인계 |

세 모듈은 한 개인의 정체성·기억·정서·목표를 읽으며 별도 인격이나 세 개의 저장 원본을 만들지 않는다. 공유하는 원본·사실·권한과 각자의 판단 목표를 구분한다. 모두 자기 목표에 따른 전체 행동 후보를 제안하고 필수 현재 입력은 같게 받는다. 첫 판단은 서로의 출력을 보지 않고 병렬 생성한다. 이후 공통 후보를 같은 snapshot에서 평가하고 모이라이가 고유 목표의 추천과 충돌을 조정·종합한다. 동일 목표를 세 번 판단하거나 모두의 동의를 강제하지 않는다. 선택 정책의 논리적 소유자는 모이라이고 Host는 그 정책의 실행·검증·선택 RNG와 receipt 저장·효과 인계를 맡는다. [016의 판단 계약](../platform/016_neural_preference_contract.md#세-판단-모듈의-계약)을 따른다.

합의는 증거를 대신하지 않는다. 목표 충돌은 선언된 조정 정책으로 처리하고, 필수 사실이나 조정 기준이 부족한 경우 추가 확인·허용된 검증·질문·보류로 처리한다. 의견 불일치 자체는 보류 사유가 아니다. 일반 대화의 기본 LLM 수명은 3+1, 명시적 행동은 3+2에 공통 후보 평가가 붙는다. 필요한 추가 LLM 호출·도구·신경 계산 비용을 모두 기록한다. 판단 모듈은 허용된 읽기/계산 포트를 사용하고 저장·외부 효과·사용자 발송은 Host와 기존 owner가 수행한다.
...
프리셋은 `presetId`, 불변 `revision`, 역할·티어별 정확한 model/provider, 추론·입출력·동시 실행 설정, 호환 SDK/API 버전, 검증 증거를 가진다. 개발 단계의 후보와 제품에서 지원하는 프리셋을 구분한다. 현재 GLM Flash는 R0 개발 후보이며 인지 품질이 검증된 제품 기본값으로 확정하지 않았다.
