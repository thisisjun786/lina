...
2026-09-11 후속 연구는 동일 모델 안에서 역할별 판단 방법을 분화하고 모델별 프롬프트 묶음을 최적화하는 데 집중한다. [연구 근거와 비교 실험](032_moirai_prompt_research.md), [네 역할의 지침 후보](033_moirai_prompt_candidates.md)를 바탕으로 [QA C/E 비교 경로](040_moirai_prompt_comparison.md)를 구현했다. 로컬 합성 provider의 실행 계약을 검증하며, 제품 적용과 인지 성능 검증은 아직 수행하지 않았다.
