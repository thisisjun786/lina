...
세 주체의 출력 수신자는 사용자 아닌 모이라이다. 각자 전체 상황에 대한 판단·추천 행동·근거·중요한 불확실성이나 판단 변경 조건을 짧은 의견으로 전달하며, 완성된 사용자 답변 세 개를 작성하지 않는다. 상세한 사고 독백이나 형식적인 반론을 요구하지 않는다. 모이라이는 출처가 표시된 세 의견을 원래 입력에 대조해 판단하고 사용자용 응답을 작성한다. 사용자용 말투·길이·형식 조건은 최종 응답에 적용하며, 내부 의견을 그대로 중계하지 않는다.
