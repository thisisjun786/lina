...
세 모듈은 전체 상황에 대한 판단·추천 행동·근거·중요한 불확실성이나 판단 변경 조건을 내부 조언으로 전달하며, 완성된 사용자 답변 세 개를 작성하지 않는다. 필요한 깊이는 판단할 문제에 맞추고 공통 문장 수 제한이나 형식적인 반론을 강제하지 않는다. 시스템 프롬프트는 영어로 작성하고 이름을 연기하는 지시를 넣지 않는다. 출력 언어는 사용자의 현재 발화와 명시적 언어 요청을 따른다.
