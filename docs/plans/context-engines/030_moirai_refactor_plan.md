...
상태: R0의 QA 전용 조정기와 실험 게이트웨이를 구현했다. 네 지속 Codex 스레드로 GLM 3판단·1종합과 완료 회차 뒤 재시작을 검증했다. [R0 실행 증거와 한계](031_moirai_r0_evidence.md)를 참고한다. 제품 통합·인지 성능·qualification은 미완료다.

실행 엔진 선택은 재검토 중이다. 2026-09-10 사용자는 퇴역한 Senpi 구현을 되살리지 않고 [새 Senpi SDK를 격리 검증](../../../scripts/qa/senpi-sdk/README.md)하는 방향을 승인했다. 현재 제품 실행은 여전히 Codex이며, 아래 Codex 스레드 설계는 제품 전환이 확정된 계약이 아니다. SDK 호환성과 세 판단 구조의 인지 효용은 별도로 검증한다.
...
| 신규 계약 | 내용 |
| --- | --- |
| ChannelBinding | channelId, agentId, 역할별 threadId, bindingGeneration, presetId/presetRevision, 실제 모델·provider와 설정 revision |
...
Codex 후보에서 프로세스 하나의 네 thread와 별도 프로세스의 thread 네 개 중 배치 방식은 미확정이다. 스레드 수와 프로세스 수를 동일시하지 않는다. 실제 설치 버전의 동시 실행·이벤트 분리·격리·비용으로 선택한다. 새 Senpi SDK 검증 결과로 실행 엔진을 결정하기 전에는 Codex 고유 binding·복구 구조를 제품에 확대 적용하지 않는다. 별도 provider 직접 호출 백엔드는 이번 SDK 검증 범위에 넣지 않는다.
...
프리셋은 `presetId`, 불변 `revision`, 역할·티어별 정확한 model/provider, 추론·입출력·동시 실행 설정, 호환 native/API 버전, 검증 증거를 가진다. 개발 단계의 후보와 제품에서 지원하는 프리셋을 구분한다. 현재 GLM Flash는 R0 개발 후보이며 인지 품질이 검증된 제품 기본값으로 확정하지 않았다.
...
이는 후속 제품 계약이다. 현재 제품의 자유 profile/tier 설정 API가 제거됐다는 뜻은 아니다. R4에서 현재 `models/types.ts`, `validation.ts`, `settings.ts`, `selection.ts`, 설정 HTTP/UI와 OpenCodex 송신 소비자를 함께 전환한다. 이전 모델 온보딩 문서의 자유 선택 제안보다 이 계약을 우선한다.
