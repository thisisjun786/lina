...
2026-09-11 · `moirai-role-methods-v0` · QA C/E 적용, 모델별 최적화·실모델 평가·제품 적용 전
...
각 판단 호출에는 공통 지침, 해당 역할 지침, 모델별 표현 revision, Host가 만든 현재 입력 묶음, 해당 응답 계약을 제공한다. 아래 블록들은 함께 사용한다. 블록 하나만 떼어 완성된 제품 프롬프트로 쓰지 않는다. [QA 비교 경로](040_moirai_prompt_comparison.md)는 아래 원문과 별도의 JSON validator를 함께 연결한다. 기본 R0 경로는 자연어 응답을 유지한다.
