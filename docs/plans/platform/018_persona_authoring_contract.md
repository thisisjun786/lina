...
| 작성 정체성 | 이름·역할·성격·말투·전기·외형·관심사, 확정된 온보딩 장(정체·가치·기질·관심·관계·표현), 대화 방식과 예시 | [`AgentProfile`](../../../packages/lina-core/src/agents/types.ts#L3), [확정 장](../../../packages/lina-core/src/onboarding/types.ts#L3), [저장된 대화 설정](../../../packages/lina-core/src/agents/conversation.ts#L26), [프리셋 기본 대화 설정](../../../packages/lina-runtime/src/persona/conversation.ts#L13) | [핵심 권위 문구](../../../packages/lina-core/src/agents/persona.ts#L38), [세 출처를 함께 렌더링](../../../packages/lina-core/src/agents/persona.ts#L167) |
...
작성 정체성은 한 곳에서 오지 않는다. `corePrompt`는 `AgentProfile` 필드와 대화 방식·예시, 확정된 온보딩 장을 한 블록에 렌더링하고 그 전체를 완전한 작성 정체성이라고 선언한다([조립 지점](../../../packages/lina-core/src/agents/persona.ts#L167)).

대화 방식과 예시의 출처는 둘이다. 그 에이전트의 저장된 설정이 있으면 ConversationStore의 값을 쓰고, 없으면 `data/personas/conversations.json`의 프리셋 기본값을 쓴다([선택 지점](../../../packages/lina-runtime/src/persona/hooks.ts#L54), [기본값 읽기](../../../packages/lina-runtime/src/persona/conversation.ts#L13)). 프리셋으로 만든 에이전트는 저장된 행이 없으므로 기본값 쪽이 프롬프트에 들어간다. 작성자가 대화 방식을 바꾸려면 어느 쪽을 고치는지 먼저 확인해야 한다. 확정 장은 OnboardingStore에서 온다([확정 장 공급](../../../packages/lina-runtime/src/fleet/manager.ts#L355)). 이들 모두 작성물이고 학습이 덮어쓰지 못한다.
