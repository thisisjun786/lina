...
상태: 2026-09-08 사용자가 작업 재개를 요청했다. 010~060은 로컬 구현·검사·독립 리뷰를 마쳤다. [070 이미지·프로필 사진 연결](070_images_and_avatars.md)은 기존 이미지 엔진 연결, 예약 복구, 자동 실행, 관리 API와 HTTP 검증을 진행하고 있다. 독립 리뷰에서 발견한 일시정지 후 재개 문제를 수정한 뒤 최종 검증을 마친다. [080 전체 인수 검증](080_surfaces_and_acceptance.md)은 아직 시작하지 않았다. 기존 PR #3은 완료 전 체크포인트이며, 머지·배포·유료 호출은 허용 범위 밖이다.
