...
상태: 2026-09-07 전체 구현 진행 중. 이 문서는 구현 순서와 완료 기준을 정한다. 첫 저장·조회·전달 단위는 로컬 구현·검사·독립 리뷰를 통과했다. [세계 편집 단위](020_world_authoring.md)와 [사회 엔진 단위](030_social_engine.md)도 구현과 독립 검토를 마쳤다. LIFE 전체 완료를 뜻하지 않는다.
...
010~030의 테스트는 구현됐으며 각 단계 문서에 실행 증거가 있다. 040~080에 적힌 후속 테스트는 현재 실행 가능한 검사라고 부르지 않는다. 구현 전에 실패 테스트를 만들고 실행해 실패 이유를 확인한다. 이후 해당 테스트와 저장소가 요구하는 타입·lint·관련 회귀 검사를 실행한다. 최종 통합 때는 [POLICY](../../../POLICY.md)의 현재 체크를 적용한다.
