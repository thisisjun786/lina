...
상태: 2026-09-07 계획안. 이 문서는 구현 순서와 완료 기준을 정한다. 현재 최소 월드 엔진이 LIFE 전체를 구현했다는 뜻은 아니다.

로드맵 검토를 마쳤으며, 사용자 요청에 따라 이후 구현 단위를 PABCD 순서로 진행한다. 아래 ‘이번 작업의 계약’은 먼저 수행한 계획 작성 범위다. 구현은 각 단계의 검증 기준과 기존 이미지·UI 담당 경계를 따르며, 머지·배포·유료 생성의 권한은 확대하지 않는다.
...
| 항목 | 내용 |
| --- | --- |
| 방식·계기 | 초기 일상·SNS 기획을 빠짐없이 구현하기 위한 요구 충족형 계획. 사용자 요청은 **계획 먼저**다. |
| 목표 | 사용자가 만든 배경 안에서 에이전트가 살아가며 경험·비밀·관계·성격을 쌓고, 실제 업무가 이후 사건에 영향을 주며, LIFE 게시물·이미지·상호작용으로 이를 볼 수 있다. |
| 제외 | 이번 작업의 제품 코드 변경, 실행 예약, 실제 생성 호출, 푸시·머지·배포. 제품 범위에서도 외부 X 계정 운영·3D 게임·새 대화 실행 엔진은 포함하지 않는다. |
| 검증 | 아래 요구 추적표와 단계별 실패 시나리오를 대조하고 문서 링크·변경 범위를 검사한다. 기존 테스트는 기반 동작만 관찰하며 새 LIFE 기능의 증거가 아니다. |
| 종료 | 모든 단계의 변경 파일·저장 경로·소비자·실패 처리·완료 기준이 문서화되고 미정 제품 결정이 표시되면 계획 작업을 마친다. |
...
각 단계에서 추가할 테스트 파일은 아직 없으므로 현재 실행 가능한 검사라고 부르지 않는다. 구현 전에 실패 테스트를 만들고 실행해 실패 이유를 확인한다. 이후 해당 테스트와 저장소가 요구하는 타입·lint·관련 회귀 검사를 실행한다. 최종 통합 때는 [POLICY](../../../POLICY.md)의 현재 체크를 적용한다.
