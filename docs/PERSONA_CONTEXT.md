...
2026-09-08 · Lina 개발자를 위한 연구 및 적용 기록

## 현재 구현

네이티브 기억 엔진이 개인 경험의 근거를 보관하고, 페르소나 엔진은 작성된 정체성을 유지하면서 적격 경험을 현재 대화에 반영한다. 공유 가능한 개인 성향은 별도 해석으로 trait/habit 값만 생성한다. AgentStore의 원본 참조·해석 receipt와 LIFE의 허구 경험은 각 소유자에 남는다. 일반 대화와 LIFE가 같은 합성 행동을 사용해도 사건·비밀·개인 문장을 자동 공유하지 않는다.

재시작 복구·중복 해석·철회·구버전 입력 보존은 합성 데이터와 실제 임시 DB로 검증했다. 실제 모델의 성격 해석 품질과 장기 대화는 아직 별도 검증 대상이다. 상세 계약과 범위는 [개인 성장 투영](plans/context-engines/031_persona_projection.md)을 따른다. 아래 9월 6일 조사 기록의 외부 엔진 유지 방침은 [자체 엔진 흡수 계획](plans/context-engines/000_plan.md)으로 대체됐다.

## 컨텍스트 실행

원문은 요약 뒤에도 보존하며, 요약은 원문과 하위 요약을 참조한다. 정책·모델 경로·추정 방식·실제 입력을 저장해 재시작 뒤에도 적격 캐시만 재사용한다. 정책이 바뀌면 새 대화가 없어도 재구성을 시도하고, 실패 시 이전 유효 요약을 유지한다. 일반 요청은 작업 상태·기억·요약·최근 원문 중 예산 때문에 제외된 부분을 구분해 보고한다. 원문의 안정 ID를 알 수 없는 native 입력에는 최근 원문을 추측해서 중복 삽입하지 않는다. 상세 설정과 검증 범위는 [041](plans/context-engines/041_context_execution.md)을 따른다.

## 결정

Lina는 사람과 대화하고 일을 조율하는 독립 에이전트다. 현재 실행은 Codex가 맡으며, 업무에는 개발·비개발·단순 검색이 모두 포함된다. 대화의 연속성, 기본 인격, 사용자의 취향, 현재 기분을 하나의 거대한 SOUL 문서에 섞지 않는다. **기본 인격과 말투는 매 요청의 시스템 지침으로 유지하고, 성장 상태와 기억은 출처·수명·예산을 가진 별도 맥락으로 붙인다.**
