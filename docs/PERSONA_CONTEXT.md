...
2026-09-09 · Lina 개발자를 위한 연구 및 적용 기록
...
World Info의 선택적 삽입은 외형·설정·기억을 상시 지침에서 분리하는 참고가 된다. 하지만 관련 단어가 나왔다는 이유만으로 설정을 모두 넣지는 않는다. LINA는 외형·프로필을 요청 시 읽고, 기억은 내장 기억 엔진에서 조회한다. 세계 사건은 별도의 공개 범위를 검사한다. [World Info](https://docs.sillytavern.app/usage/core-concepts/worldinfo/)
...
Open-LLM-VTuber는 중단 시 작업을 취소하고, 사용자가 들은 응답 부분과 중단 표시를 기록한다. LINA의 텍스트 대화에서도 전송 중 내용, 완료한 답변, 중단된 답변의 의미를 구분한다. 불확실한 요청을 자동 재전송하지 않으며, 기억 학습에는 완료 여부와 원문 출처가 검증된 입력을 사용한다. 음성·끼어들기 UI는 이 범위에 포함하지 않았다. [중단 처리](https://github.com/Open-LLM-VTuber/Open-LLM-VTuber/blob/992309c0aa19845960228f880013d4685fde93b5/src/open_llm_vtuber/conversations/conversation_handler.py)

해당 프로젝트의 기본 memory agent는 대화 버퍼와 중단 처리를 제공하지만 LINA가 요구하는 장기 기억 전체를 대체하지 않는다. LINA는 기억 추론과 원문·요약 복원을 자체 엔진에서 관리한다. [기본 memory agent](https://github.com/Open-LLM-VTuber/Open-LLM-VTuber/blob/992309c0aa19845960228f880013d4685fde93b5/src/open_llm_vtuber/agent/agents/basic_memory_agent.py)
...
| --- | --- | --- | --- |
| 실행 지침 | 실제 도구·기억·작업 실행의 동작, 앱 작성자 | 시스템 지침, 서버 버전 수명 | 실제 권한 코드는 별도 |
...
| 업무 맥락 | 목표·결정·미완료 작업과 원문 연결 | 세션 컨텍스트·공유 자료의 범위별 조회 | 최신 사용자 방향 우선 |
| 사용자 기억 | 해당 에이전트의 내장 기억 조회 결과 | 해당 요청, 정책별 조회·입력 한도 | 추론은 참고이며 현재 지시보다 낮음 |
...
flowchart TD
    ...
    C --> D[세션 컨텍스트 + 개인 기억 조회]
    D --> E[Codex instructions와 대화 input으로 전송]
    E --> F[대화 응답 또는 허용된 작업 실행]
    F --> G[완료한 대화 저장]
    G --> H[내장 기억 관찰 · 재검토]
...
아래 수치는 9월 6일 검증 당시 값이다. 현재 컨텍스트·기억의 적용 한도는 엔진 정책과 모델 설정을 읽어 결정하며 [상태 계약](plans/context-engines/004_contracts.md)과 [컨텍스트 실행](plans/context-engines/041_context_execution.md)이 기준이다. 아래 Honcho 서버 장애 설명은 퇴역한 외부 어댑터의 기록이다.
...
아래 실제 모델 요청은 최초 대화 스타일 구현 당시의 증거다. 이후 자체 기억·성장·공유 자료 통합 변경까지 검증한 결과가 아니며, 현재 버전의 실제 모델 자격 검증은 별도로 남아 있다.
