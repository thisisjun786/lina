...
The [Moirai refactor](plans/context-engines/030_moirai_refactor_plan.md) keeps engine activity behind the ordinary conversation and replaces user-composed backend/model settings with maintainer-qualified presets. The [R0 QA probe](plans/context-engines/031_moirai_r0_evidence.md) exercises four persistent native threads separately from the product. The [C/E prompt comparison](plans/context-engines/040_moirai_prompt_comparison.md) adds a QA-only single round with fresh threads, neutral inputs, frozen prompts, strict judgment validation and complete failure records. Its local scripted provider checks execution contracts, not model quality; valid invalidation withholds synthesis. Preset enforcement and the new conversation boundary remain planned; the existing model APIs described below still apply to the current runtime.
...
| lina-memory | Native evidence-linked memory and scoped non-code resources, derivations and shared knowledge |
...
The Lina-owned resource catalog stores non-code documents and logical collections independently of tasks. Stable `lina://resources/<uuid>` addresses, immutable content versions, host-supplied visibility scopes, durable extraction/summary jobs and source-checked search live in `lina-memory/src/resources`. `ResourceEngine` composes scoped tools, bounded model calls and explicit recovery; closing it aborts and drains active work before closing SQLite. Fleet now owns one resource installation with agent-scoped consumers, an activity ledger, HTTP/tools and checkpoint recovery. OpenViking's runtime adapter is retired; historical external data is not automatically imported. See [the resource contract](plans/context-engines/051_resource_execution.md).

HTML resources use bounded static text extraction with no JavaScript or network execution. Activity evidence uses the current derived extract, preserving the original bytes separately. Script/style/template contents and inline-hidden elements are excluded; computed CSS visibility is outside this extractor. This resource-specific support does not widen the chat attachment MIME contract.
...
Authored core identity, inferred preferences, temporary mood, confirmed user corrections and shared resource knowledge have distinct owners and provenance. Existing profiles are not overwritten from seed presets. Native memory is the default; disabled learning retains local data, while a legacy Honcho selection reports migration required without external I/O. Write acceptance, derivation/indexing and later recall are separate outcomes. Retrieved content is reference data, not new instructions.
...
Session context uses the same persisted engine-policy owner as memory. Version-1/2 policy payloads keep their original bytes/revision when read as version 3; legacy edits preserve newer context and resource settings. Context policy controls summary input/output, refresh threshold, retained tail, per-request search/expansion limits and a separate total injection budget. The shared UTF-8/2 heuristic is an estimate, not a tokenizer or guaranteed upper bound; native compaction acceptance still uses its own fit check.
...
Authored identity is preserved. Ordinary Lina sessions may receive permitted shared personality and relationship changes; world-event recall and factual-memory capture require separate scoped bindings and consent. Work can contribute through task provenance or explicitly granted resource activities without a task ID. Recorded activity is distinct from a host-verified result. Corrections, revoked grants and changed source versions invalidate pending consumption. LIFE fiction does not become ordinary factual memory automatically.
