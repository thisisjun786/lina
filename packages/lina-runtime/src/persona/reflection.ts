...
export class PersonaReflection {
	...
	settled(): void {
		...
		if (
			...
			return;
		const dynamics = agents.dynamics(agentId);
		...
		this.running = (async () => {
			...
			const payload = {
				...
				userCommunicationPreferences:
					this.options.conversations?.getPreferences(agentId).items ?? [],
				allowCharacterGrowth:
					!this.options.preferencesOnly && profile.evolution === "adaptive",
				provisionalCharacterGrowth: agents.pendingGrowth(agentId),
				sourceEntryId: source.entryId,
				userMessage: source.text.slice(0, 6000),
				recentConversation: history.map((e) => ({
					id: e.entryId,
					role: e.role,
					text: e.text.slice(0, 2000),
				})),
				honchoReference: recall,
			};
				...
				try {
					this.options.conversations.observePreferences(
						...
						(entryId) => {
							...
							return entry?.role === "user" && entryId === source.entryId
			...
			if (!this.options.preferencesOnly)
				agents.applyReflection(
					...
					(id) => journal.entry(id)?.role === "user" && id === source.entryId,
				);
