...
interface Options {
	store: Pick<WorldStore, "imageAttempts" | "imageIntent">;
	discovery: Pick<LifeImageDiscovery, "preview" | "freeze">;
	images: Pick<LifeImages, "run" | "resume" | "reconcile" | "read">;
	clock: LifeClock;
	foreground(): boolean;
...
export async function visitLifeImages(
	...
	for (const attempt of store.imageAttempts(worldId)) {
		...
		if (state === "completed") {
			try {
				options.completed(images.read(worldId, attempt.attemptId));
			} catch (error) {
