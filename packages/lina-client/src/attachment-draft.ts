...
export class AttachmentDraft {
	...
	set value(value: string) {
		...
		if (refs.length > MAX_DRAFT_ATTACHMENTS) {
			this.text = value;
			this.items = [];
		} else {
	...
	add(ref: DraftAttachment, index = this.items.length): void {
		if (
			this.items.some(
		...
		if (this.items.length >= MAX_DRAFT_ATTACHMENTS)
			throw new Error("첨부 한도 초과 · 최대 4개");
		const previous = this.items;
		this.items = [...previous.slice(0, index), ref, ...previous.slice(index)];
