...
} from "./model-combobox-types.ts";
import type { SettingsDocument, SettingsNode } from "./settings-dom.ts";

// Existing task and hub controllers consume these structural DOM contracts.
export type { SettingsDocument, SettingsNode } from "./settings-dom.ts";
...
export function installModelSettings(
	...
	const rows = new Map<
		RowRole,
		{
			combo: ReturnType<ModelComboboxFactory>;
			reason: SettingsNode;
			hint: SettingsNode;
		}
	>();
	...
	for (const role of ["default", ...MODEL_ROLES] as const) {
		...
		const combo = combobox(doc, `model-${role}`, `${label} 모델`, (value) => {
