import type { ExecutionModel } from "../../lina-client/src/execution-model.ts";
...
export function createExecutionView(
	source: ExecutionModel | (() => ExecutionModel),
	...
	stop.addEventListener("click", () => dispatch(getModel().cancelCommand()));
	return () => {
		const model = getModel();
		if (bound !== model) {
