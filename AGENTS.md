...
- Current runtime execution is Codex: `lina-codex` owns its RPC adapter and OpenCodex owns provider routing. The confirmed engine design selects Senpi as the cognition backend and keeps the Codex TaskManager as the development-task execution owner; that switch lands through the F1–F4 roadmap, not ad-hoc edits. `lina-runtime/src/host.ts` contains Lina-owned contracts; do not introduce an engine SDK dependency into channels or memory.
